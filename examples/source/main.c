...
int main(){
	...
	while(run) {
		...
		if ((pressed & WPAD_BUTTON_RIGHT || pressed & WPAD_BUTTON_2 || pressed & WPAD_BUTTON_PLUS || pressed & WPAD_BUTTON_DOWN) && n+1<100){
			clearScreen();
			n+=1;
			showMii(miis[n]);
			...
			clearScreen();
			char filename[64];
			snprintf(filename, sizeof(filename), "mii_%02d.bin", n);
			int r = MiitoSD(n, &miis[n], filename);
			if (r == 0) printf("Saved Mii %d to %s\n", n, filename);
